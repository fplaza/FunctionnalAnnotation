...
def get_parameters():
	...
	parser.add_argument('-b', '--best-hit', dest='best_hit', action="store_true", default=False,
			help='Assign gene to the KO corresponding to the best hit.')
...
def extract_functionnal_annotation(kegg_gene_to_ko, hits_file, best_hit):
	query_gene_to_ko=OrderedDefaultDict(set)
	with open(hits_file, 'r') as istream:
		for line in istream:
			query_gene, kegg_gene = line.split()[:2]
			if not best_hit or query_gene not in query_gene_to_ko:
...
def write_functionnal_annotation(query_gene_to_ko, functionnal_annotation_file):
	with open(functionnal_annotation_file,'w') as ostream:
		for query_gene in query_gene_to_ko:
			for ko_annotation in query_gene_to_ko[query_gene]:
				print('{0}\t{1}'.format(query_gene,ko_annotation), file=ostream)
...
def main():
	...
	query_gene_to_ko=extract_functionnal_annotation(kegg_gene_to_ko, parameters.hits_file, parameters.best_hit)
